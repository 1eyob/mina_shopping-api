...
module.exports={
    createUser: async (data,callback)=>{
        ...
        if(data.fullname==undefined || data.phonenumber==undefined || data.password==undefined || data.address==undefined){
                    ...
                    if(error){
                        ...
                        if(status){
                            ...
                            if( result[0].coverimage=='') {
                                ...
                                result[0].coverimage=process.env.APP_HOST+"/user/images/"+result[0].coverimage
                            }
                            
                            result[0].profileimage=process.env.APP_HOST+"/user/images/"+result[0].profileimage
                            return callback(null,result)
                        }
                        else{
                            return callback("incorrect username or password")
    ...
    changePassword: async (data,callback)=>{
        if( data.password==undefined||data.phonenumber==undefined||data.newpassword==undefined){
            return callback("please send data with along correct attribute")
         }
...
getUserById: async (data,callback)=>{
    ...
    editProfile: async (data,req,callback)=>{
        ...
        if(req.file!=undefined && req.file.fieldname!=undefined &&  req.file.fieldname!=null && req.file.fieldname!=""){
    ...
    followUser: async (data,callback)=>{
        ...
        await pool.query(queryStatment,values,(error,result)=>{
            if(error){
                ...
                createNotification({touser:data.following,sentfrom:data.follower,text:`start following you`},(error,result)=>{
                  if(error){}else{}
                })
                return callback(null,result)
            }
            ...
            if(error){
                return callback(error)
                   ...
                   for(var i=0;i<result.length;i++){
                    result[i].profileimage=process.env.APP_HOST+"/user/images/"+result[i].profileimage
                   }
                    return callback(null,result)
    ...
    getAllusers: async (data,callback)=>{
        ...
        var queryStatment=`select userid ,profileimage, fullname from users`
